...
export default function Button({ children, type = "button", variant = "primary", isDisabled, className, onClick }: ButtonProps) {
  const variants = {
  ...
  return (
    <button type={type} onClick={onClick} disabled={isDisabled} className={cn(variants[variant], className)}>
      {children}
    </button>
  );
}
